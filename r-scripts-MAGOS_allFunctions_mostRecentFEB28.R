###DR Liu s final edits. USE THESE FROM DEC 20. NEW NAME. NEW EDITS
###   Added mag.reduce functions
###   Added mag.multiple cut to take x.el as input

####LOOK AT FEB 28 updates. (added ID, new cut.off)
...
optim.obj <- function(shapes, y, weight=-0.0001){     #org april uses this one
	R <-dbeta(y, shape1=shapes[1], shape2=shapes[2], log=F)
	R[which(R < 1e-10)] <- 1e-10;
	if(length(which(R == 1e-10)) > 0) {
		return(10000);
	}
	print('y')
	print(y)
...
##### edit on jan 18 to optimize time.
fit.variants.weight <- function(variants){   #to calculate the initial matrix
  weight <- variants[length(variants)]
  variants <- variants[-length(variants)]
  fit<-optim(c(global.s1, global.s2), y=variants, fn=optim.obj, weight=weight, lower=c(1.01, 1.01), upper=c(1000, 1000), method='L-BFGS-B', control=list(factr=1e-3, maxit=100, trace=0))
  global.s1 <- fit$par[1]
  global.s2 <- fit$par[2]
  return(c(fit$value, fit$par[1], fit$par[2]))
} #ORIGINAL before jan 18
###april 9th 2018 version
fit.variants.weight <- function(variants){   #to calculate the initial matrix
	weight <- variants[length(variants)]
	variants <- variants[-length(variants)]
	fit<-optim(c(global.s1, global.s2), y=variants, fn=optim.obj, weight=weight, lower=c(1.01, 1.01), upper=c(1000, 1000), method='L-BFGS-B', control=list(factr=1e-3, maxit=0, trace=0))
	global.s1 <- fit$par[1]
	global.s2 <- fit$par[2]
	return(c(fit$value, fit$par[1], fit$par[2]))
}
...
#########MAY 18th
fit.elements.weight <- function(element.1, element.2, arg.data, ss.other){   
  data.1 <- arg.data[as.logical(element.1)]
  data.2 <- arg.data[as.logical(element.2)]
  weight <- get.weights.within(element.1 + element.2, ss.other)
...
fit.elements.weight.reduce <- function(element.1, element.2, arg.data, ss.other){   
  data.1 <- arg.data[as.logical(element.1)]
  data.2 <- arg.data[as.logical(element.2)]
  weight <- get.weights.within(element.1 + element.2, ss.other)
  data.merge <- c(data.1, data.2, 10*weight)
...
get.weights.within <- function(x1, ss) {   #ss should be the remaining samples
	weights <- c()
	#print(ss)
	if(sum(x1)>1){  
		weights <- apply(ss, 2, function(x){ return(max(dist(x[as.logical(x1)]))) })
	} else{
		weights <- 0
	}  
	#print(weis)
...
# ss should be the remaining samples
get.weights.between <- function(x1, x2, ss) {  
	xx <- x1;
	if(is.vector(x1)) {
		xx <- t(as.data.frame(x1));
	}
	weights <- apply(xx, 1, function(x) { x <- x + x2; return(get.weights.within(x, ss=ss)) } );
	return(weights)
}
...
mag.single <- function(arg.data){      
	...
	while(nrow(x.elements)>= 2){
		...
		# print("Remove ina")
		# print(el.rm)
		params<-rbind(params, cbind(par.1.loop[el1,el2], par.2.loop[el1,el2]))
		...
		var.mean <- rbind(var.mean, t(apply(x.elements, 1, get.var, x.nb=sort.x, s=s )))          ####for oc.v9 I changed x.nb=x.nb to x.nb=
...
#			print("if3")
		...
		if(length(nei) > 0){
			...
			temp <- apply(nei, 1, fit.elements, arg.data=x.loop, element.2=x.last)
			...
			par.1.loop<-cbind(par.2.loop, par1.column.update)
	...
	result <- list(x.el=x.elements.list, params=params, var.mean=var.mean, freq=x.nb, freq.s=sort.x, time=list(time0,time1, time2))
	return(result)
}

#### add ids to mag.single.  arg.data should have ID column     APRIL 15 2018    
...
#	print(allPoints)
	...
	while(flag == FALSE){
		...
		if(sum(ok.points & stepI.x.el.step) == 0){     ###meaning stepI.x.el is not in ok.points 
			...
			stepII.num <- stepII[, 4]
			stepII.breaks <- cbind(stepII.breaks, stepII[1,4])   #just add the step number back to the matrix
...
#				if(stepII.breaks[ii, 'NumberOfPoints'] == 1 | stepII.breaks[ii,2] < m+vv) {        #okay cluster
				if(stepII.breaks[ii, 'NumberOfPoints'] == 1 | stepII.breaks[ii,2]*(1+stepII.breaks[ii, "weights"]) < m+vv) {        #okay cluster
	...
	results<-list(str=str, cutclust=cutstr, cutclust2=cutstr2, colors=colors)
	return(results)  
}
####updated dec 19 2017:
find.cut.off <- function(mag.output, shape, depth){
  ...
  while(flag == FALSE){
    ...
    if(sum(ok.points & stepI.x.el.step) == 0){     ###meaning stepI.x.el is not in ok.points 
      ...
      for(ii in 1:2){  
        ...
        #       if(stepII.breaks[ii, 'NumberOfPoints'] == 1 | stepII.breaks[ii,2] < m+vv) {        #okay cluster
        print('NUMMMMMSSSS')
        print(stepII.breaks[ii, 'NumberOfPoints'])
...
{
mag.reduce<- function(arg.data, sampleNum=1){ 
  ###### arg.data has more than 100 mutations.
  #sampleNum<-1
  one.sample<- mag.single(arg.data[,sampleNum, drop=F])
  one.cut<-find.cut.off(one.sample, depth=200, shape=7)       ##### add depth and shape
  row.names(arg.data.reduce)
  
  
  arg.data[ order(arg.data[,sampleNum]),]
  arg.data.reduce<- cbind(arg.data[ order(arg.data[,sampleNum]),], colors= one.cut$colors)
  ### arg.data.reduce has all the points with the one sample clustering. 
  
  one.clusters<-unique(one.cut$colors)
  x.el.raw<- data.frame(diag(nrow(arg.data)))
  row.names(x.el.raw)<- row.names(arg.data.reduce)
  colnames(x.el.raw)<- row.names(arg.data.reduce)
  count<-0
  x.el.final<- data.frame()
  ### i will add their weights to each cluster
  #i<-1
  #i<-3
  for( i in 1:length(one.clusters)){
    cluster.num<-one.clusters[i]
    count<- c(count,length(arg.data.reduce[arg.data.reduce$colors==cluster.num, "colors"]))
    
    
    cut<- arg.data.reduce[ arg.data.reduce$colors==cluster.num,]
    cut.ind<- which(arg.data.reduce$colors==one.clusters[i])
    l<- dim(cut)[1]
    
    #x.el.cut<- data.frame(diag(nrow(cut) ))
    x.el.cut<- x.el.raw[(sum(count[-(i+1)])+1):(sum(count[-(i+1)])+count[i+1]),]
    
    if(nrow(cut)>1){
      
      indx<-c(1:length(cut[,1]))
      combn.idx <- t(combn(indx, 2))
      weights <- apply(combn.idx, 1, function(x) { temp.elements <- rep(0, l);
      temp.elements[x] <- 1; return(get.weights.within(temp.elements, ss=data.frame(cut[,2]))) } );  ### Now I Have the initial weights	
      combn.x <- t(combn(cut[,1], 2))	   
      initial <- cbind(combn.x, weights)
      fit.initial <- t(apply(initial, 1, fit.variants.weight))
      
      tempMatLoop<-lower.tri(diag(0, nrow=l, ncol=l))
      
      tempMatLoop[tempMatLoop] <- fit.initial[, 1]
      ...
      while( min(mat.loop)< -10 & nrow(mat.loop)>2 ){
        ...
        #el.em<- which(mat.loop < -10, arr.ind=T)
        #update x.el.cut, mat.loop
        #if( min(mat.loop)< -10){
        mat.loop<- mat.loop[-el.rm, -el.rm, drop=F] 
        if(sum(dim(mat.loop))==0){mat.loop=matrix(c(0,0,0,0),nrow = 2)}
        # x.el.cut<- rbind( x.el.cut[-el.rm, ], x.el.cut[el.rm[1],]+x.el.cut[el.rm[2],])
        x.el.cut<- rbind( x.el.cut[-el.rm, , drop=F], colSums(x.el.cut[el.rm,]))
        colnames(x.el.cut)<-row.names(arg.data.reduce)
        
        
        el.rm.raw<-el.rm+sum(count[1:i])
        print(el.rm.raw)
        #x.el.raw<- rbind(x.el.raw[ -el.rm.raw, ], x.el.raw[el.rm.raw[1],]+ x.el.raw[el.rm.raw[2],])
        
      }}
    print(x.el.cut)
    x.el.final<- rbind(x.el.final, x.el.cut)
  }
  ...
  x.el.raw
  arg.data.reduce
  results<- list(x.el= x.el.final, one.sample= arg.data.reduce)
  return(results)
}

mag.reduce.fast<- function(arg.data, sampleNum=1){
  x.el.raw<- data.frame(diag(nrow(arg.data)))
  row.names(x.el.raw) <- row.names(arg.data)
  colnames(x.el.raw)  <- row.names(arg.data)
  x.el.final<- data.frame()
  ...
  combn.idx <- t(combn(indx, 2))
  weights <- apply(combn.idx, 1, function(x) { temp.elements <- rep(0, l);
  temp.elements[x] <- 1; return(get.weights.within(temp.elements, ss=data.frame(cut[,-sampleNum]))) } );  ### Now I Have the initial weights	
  combn.x <- t(combn(cut[,1], 2))	   
  initial <- cbind(combn.x, 4*weights)
  fit.initial <- t(apply(initial, 1, fit.variants.weight))
  ...
  while( min(mat.loop)< -10 & nrow(mat.loop)>2 ){
    ...
    # x.el.cut<- rbind( x.el.cut[-el.rm, ], x.el.cut[el.rm[1],]+x.el.cut[el.rm[2],])
    #x.el.cut<- rbind( x.el.cut[-el.rm, , drop=F], colSums(x.el.cut[el.rm,]))
    x.el.raw<- rbind(x.el.raw[-el.rm,, drop=F], colSums(x.el.raw[el.rm,]))
    colnames(x.el.raw)<-row.names(arg.data)
  ...
  #x.el.raw
  #arg.data.reduce
  results<- list(x.el= x.el.final)  #changed arg.data.reduce/ dont need it
  return(results)
}

mag.reduce.1<- function( arg.data, sampleNum=1 ){
  #head(arg.data)
  l<- nrow(arg.data)
  arg.sorted<-arg.data[ order(arg.data[,sampleNum]),]
  row.fold<- c();
  x.el.out<-data.frame(diag(0,l))
  colnames(x.el.out)<- row.names(arg.sorted)
  
  full<-ceiling(l/size.threshold)
  
  
  
  for ( i in 1:full){
    start <- 1+((i-1)*size.threshold);
    end <- size.threshold+(i-1)*size.threshold;
    if(i == full) 
      end <- l;
    folds<- c(start:end);
    cut.data<- arg.sorted[folds, ]
    print("inja")
    print(folds)
    
    reduce<-mag.reduce.fast(cut.data)
    x.el.fold<-reduce$x.el
    
    row.fold<-c(row.fold, nrow(x.el.fold))
    print(row.fold)
    print(dim(x.el.fold))
    print((sum(row.fold[-i])+1):(sum(row.fold[-i])+row.fold[i]))
    print("done ija")
    x.el.out[(sum(row.fold[-i])+1):(sum(row.fold[-i])+row.fold[i]),folds]<-x.el.fold
    
  }
  x.el.out<- x.el.out[rowSums(x.el.out)>0,]
  
  results<- list(x.el.red=x.el.out, data=arg.data, data.sorted=arg.sorted)
  return(results)
  
}

mag.reduce.2<- function(reduce.out, sampleNum=1, n=-100){
  x.el<- reduce.out$x.el.red
  data<- reduce.out$data
  data.sorted<- reduce.out$data.sorted
  
  mean.frqs<-t(apply( x.el, 1, function(x){mean(data.sorted[as.logical(x),1])}))
  ...
  ss.other<- data.sorted[,-sampleNum, drop=F]
  l<- nrow(x.el)
  size.threshold
  ...
  for(i in 1:full){
    ...
    print(i)
    initial.full<-apply(combn.el, 1, function(x){fit.elements.weight( x.el.fold[x[1],], 
                                                                      x.el.fold[x[2],],
                                                                      x.nb, ss.other )})
    tempMatLoop<-lower.tri(diag(0, nrow=size.threshold, ncol=size.threshold))
    
    tempMatLoop[tempMatLoop] <- initial.full[1, ]
    mat.loop<-t(tempMatLoop)
    diag(mat.loop) <- 1e5;
  ...
  initial.rems<-apply(combn.rem, 1, function(x){fit.elements.weight( x.el.rem[x[1],], 
                                                                     x.el.rem[x[2],],
                                                                     x.nb, ss.other )})
  tempMatLoop<-lower.tri(diag(0, nrow=rem, ncol=rem))
  
  tempMatLoop[tempMatLoop] <- initial.rems[1, ]
  mat.loop<-t(tempMatLoop)
  diag(mat.loop) <- 1e5;
...
#feb 26 arg data should also be output of mag.prepdata
mag.reduce.fast<- function(arg.data, sampleNum=1){
  x.el.raw<- data.frame(diag(nrow(arg.data)))
  #row.names(x.el.raw) <- row.names(arg.data)
  #colnames(x.el.raw)  <- row.names(arg.data)
  #feb 26
  IDs<- arg.data[,dim(arg.data)[2]]
  names(x.el.raw)<- IDs
  x.el.final<- data.frame()
  
  #feb 26
  data.other<- arg.data[, -c(sampleNum,dim(arg.data)[2]), drop=F] #remove the IDs
  print(class(data.other))
  ...
  weights <- apply(combn.idx, 1, function(x) { temp.elements <- rep(0, l);
  temp.elements[x] <- 1; return(get.weights.within(temp.elements, ss=data.other)) } );  ### Now I Have the initial weights	
  ...
  while( min(mat.loop)< 0 & nrow(mat.loop)>2 ){
    el.rm<-which(mat.loop==min(mat.loop), arr.ind=T)
    el.rm<- el.rm[1,]
    #el.rm<-unique(as.numeric(el.rm))     APRIL 14: only look at one of the pairs. (same weights, but different frequencies: put together pairs with identical weights but different frequencies. , )
    #el.em<- which(mat.loop < -10, arr.ind=T)
    #update x.el.cut, mat.loop
    #if( min(mat.loop)< -10){
    mat.loop<- mat.loop[-el.rm, -el.rm, drop=F] 
...
# arg data is out put of mag.prepdata. Already has IDs.
...
mag.reduce.1<- function( arg.data, sampleNum=1 ){
  ...
  x.el.out<-data.frame(diag(0,l))
  arg.sorted<-arg.data[ order(arg.data[,sampleNum]),]
  IDs<- arg.sorted[, dim(arg.data)[2]]
  names(x.el.out)<- IDs
  
  arg.data<- as.matrix(arg.data)
  arg.data<- round(arg.data, 4)
  
  arg.data <- ifelse(arg.data<1e-9, 1e-9, arg.data)
  arg.data[,-dim(arg.data)[2]] <- ifelse(arg.data[,-dim(arg.data)[2]]>(1-1e-3), 1-1e-3, arg.data[,-dim(arg.data)[2]])
  ...
  if(full>0){
    for ( i in 1:full){
      folds<- c((1+((i-1)*size.threshold)):(size.threshold+(i-1)*size.threshold))
      cut.data<- arg.sorted[folds, ]
      #print("inja")
      #print(folds)
      cat('TIME 2',i)
      
      ###cut data should have the ids and all the samples
      reduce<-mag.reduce.fast(cut.data, sampleNum = sampleNum) #feb 26
      x.el.fold<-reduce$x.el
      print('TIME 3')
      ...
      print(row.fold)
      print(dim(x.el.fold))
      print((sum(row.fold[-(i+1)])+1):(sum(row.fold[-(i+1)])+row.fold[i+1]))
      #print("done ija")
      x.el.out[(sum(row.fold[-(i+1)])+1):(sum(row.fold[-(i+1)])+row.fold[i+1]),folds]<-x.el.fold
      
    }}
  print('TIME 4')
  
  rems<- c((l-rem+1):l)
  cut.data<- arg.sorted[rems, ]
  reduce<-mag.reduce.fast(cut.data, sampleNum = sampleNum)
  x.el.fold<-reduce$x.el
  
  row.fold<-c(row.fold,nrow(x.el.fold))
  i<-length(row.fold)
  x.el.out[(sum(row.fold[-(i)])+1):(sum(row.fold[-(i)])+row.fold[i]),rems]<-x.el.fold
...
mag.reduce.fast<- function(arg.data, sampleNum=1){
  ...
  ###   May 6th
  ###Adding threshold: 
  ### puting point that are closer than 0.05 
  optim.value.bound<- fit.variants.weight(c(0.1,0.15,0.05))[1]
...
#sampleNum means which column the clustering should be done on:
mag.multiple <- function(ss, sampleNum=1) {         
	time0<-as.numeric(Sys.time())
...
#	mat.loop.list <- list();
	...
	ss <- as.matrix(ss);
	ss <- ifelse(ss<1e-9, 1e-9, ss)
	x.nb <- ss[, sampleNum]  
	ss.other <- c();
	if(ncol(ss) == 1){
		print('executing single sample clustering....'); flush.console();
		return(mag.single(ss))
	} else {
		ss.other<- ss[, -sampleNum, drop=F]    #other samples in the data
	}
	...
	combn.idx <- t(combn(indx, 2))
	weights <- apply(combn.idx, 1, function(x) { temp.elements <- rep(0, length(x.nb)); temp.elements[x] <- 1; return(get.weights.within(temp.elements, ss=ss.other)) } );  ### Now I Have the initial weights	
	...
	tempMatLoop[tempMatLoop] <- fit.initial[, 1]
	tempPar1Loop[tempPar1Loop] <- fit.initial[, 2]
	tempPar2Loop[tempPar2Loop] <- fit.initial[, 3]
	mat.loop<-t(tempMatLoop)
	diag(mat.loop) <- 1e5;
	...
	while(nrow(x.elements) > 1){
		...
		#mat.loop.list[[s]] <- mat.loop;
		#print("WHILEFIRST")
		#print(dim(x.elements))    
    ...
    print(mat.loop)
		...
		weights.temp <- apply(x.elements, 1, get.weights.within, ss=ss.other)
		...
		par.2.loop<-par.2.loop[-el.rm, -el.rm, drop=F]
		#print("Remove MTLOOP")
		#print(dim(mat.loop))
		#print(mat.loop)
		#print(el.rm)
		#print(dim(x.elements)[1]>=2)
		#print(dim(x.elements))
		...
		fit.between <- t(apply(x1, 1, fit.elements.weight, element.2=x2, arg.data=x.loop, ss.other=ss.other))
	...
	result <- list(x.el=x.elements.list, params=params, var.mean=var.mean, wights=weights.list ,freq.s=x.nb ,freq=x.nb)
	return(result)
}


###### mag multiple cut function
# Takes x.el as an input
mag.multiple.cut <- function(ss,x.el.cut=data.frame(), sampleNum=1) {        
  ...
  if( sum(dim(x.el.cut))==0){
    ...
    x.elements<- x.el.cut
    order<-order(ss[,sampleNum])
    ss.other<-ss.other[order,,drop=F]
    x.nb<- ss[order, sampleNum]
  }
  ...
  return(get.weights.within(temp.elements, ss=ss.other)) } );  ### Now I Have the initial weights	
  combn.x <- t(combn(x.nb, 2))	   
  initial <- cbind(combn.x, weights)
  weights.list[[1]]<-initial
  #print(initial)
  ...
  #weight.1<- apply(combn.cut, 1, function(x){temp.elements<-x.el.cut[x[1],]+x.el.cut[x[2],]
  #  return(get.weights.within(temp.elements, ss=cut[,-3])) })
  #WORKS:
  #weight.2<- apply(combn.cut, 1, function(x){get.weights.within( x.elements[x[1],]+
  #                                                  x.elements[x[2],],ss.other)})
  ...
  tempMatLoop<-lower.tri(diag(0, nrow=l, ncol=l))
  tempMatLoop[tempMatLoop] <- initial.1[1, ]
  mat.loop<-t(tempMatLoop)
  diag(mat.loop) <- 1e5;
  tempPar1Loop<-lower.tri(diag(0, nrow=l, ncol=l))
  tempPar2Loop<-lower.tri(diag(0, nrow=l, ncol=l))
...
###updates feb 26
#ss should have ID. should run map.prepdata first.
mag.multiple.cut <- function(ss,x.el.cut=data.frame(), sampleNum=1) {        
  
  if(sampleNum>dim(ss)[2]-1){print("stop the algorithm and check the sampleNum; run mag.prepdata() first")}
  ...
  ss<- round(ss,4)
  ss <- ifelse(ss<1e-9, 1e-9, ss)
  ss[,-dim(ss)[2]] <- ifelse(ss[,-dim(ss)[2]]> 1-(1e-3), 1-(1e-3), ss[,-dim(ss)[2]])
  #x.nb <- ss[, sampleNum]   feb 27    I made adjustment so that without providing x.el also runs on sorted data
  ss.other <- c();
  if(ncol(ss) == 2){        ##### change '1' to '2' because the 2nd column is always id
    print('executing single sample clustering....'); flush.console();
    return(mag.single(ss[,1])) #feb 26: the second column is the IDs
  } else {
    ss.other<- ss[, -c(sampleNum,dim(ss)[2]), drop=F]    #other samples in the data     FEB26: assuming last column is ID
  }
  
  #feb 27: took these out of the if statement. So every version is done based on sorted data.
  order<-order(ss[,sampleNum])
  ss.other<-ss.other[order,,drop=F]
  data.sorted<- ss[order, ]
  x.nb<- ss[order, sampleNum]
  IDs<- ss[order,dim(ss)[2]] #FeB26
  ...
  result <- list(x.el=x.elements.list,data.sorted=data.sorted, data=ss, params=params, var.mean=var.mean, wights=weights.list ,freq.s=x.nb ,freq=x.nb)
  return(result)
}
...
### mag reduce generates the initial x.el s. mag.reduce 2 decrreases the number of rows. 
...
mag.var<- function(mag.out, sampleNum=1){
  ...
  for ( i in 1:num){
    ...
    for( s in 1:length(x.el.list)){
      ...
      weights.temp <- apply(x.el, 1, get.weights.within, ss=ss.other)
...
### data should have the id. select columns with only the mutations
...
if(length(data$ID)==0){return(print("RUN mag.prepdata"))}
  ...
  data.prep<- data
  s<- sampleNum
  red.1<- mag.reduce.1(data.prep, sampleNum=s)
  red.2<- mag.reduce.2(red.1, sampleNum=s)
  mag.out<- mag.multiple.cut(data.prep, x.el.cut = red.2$x.el.red, sampleNum=s)
 
  mag.mv<- mag.var(mag.out,sampleNum = s)
  #feb 28: replaced with new cut.off
  #mag.cut<- find.cut.off(mag.output = mag.out ,shape=shape, depth = depth)
  
  mag.cut<- find.cut.off.multiple(mag.mv, shape=shape, depth=depth)
 
  
  results<- list(final.res<-mag.cut$final.data ,data.prep<- data.prep, 
                 red.1<- red.1, red.2<- red.2, mag.out<- mag.out, mag.cut<- mag.cut, mag.mv= mag.mv )
  
  
  names(results)<- c(paste(name,".final.data", sep = ""), paste(name,".data.prep",sep = ""), 
                     paste(name,".red.1", sep = ""), paste(name,".red.2", sep = ""),
